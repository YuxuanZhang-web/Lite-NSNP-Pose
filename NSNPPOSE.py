...
from models.DCASP import DCASP
...
class Decoder(nn.Module):
    def __init__(self, low_level_inplanes, planes, num_classes):
        ...
        self.last_conv = nn.Sequential(nn.ReLU(),
                                       nn.Conv2d(planes+reduction, planes, kernel_size=3, stride=1, padding=1, bias=False),
...
class NSNPPOSE(nn.Module):
    def __init__(self, cfg, **kwargs):
        self.inplanes = 64
        extra = cfg.MODEL.EXTRA
        super(NSNPPOSE, self).__init__()
        ...
        self.DCASP = DCASP('SEPARABLE', 48, 48, cfg.MODEL.NUM_JOINTS)
    ...
    def forward(self, x):
        ...
        x = self.DCASP(y_list[0], low_level_feat)
...
def get_NSNPPOSE(cfg, is_train, **kwargs):
    model = NSNPPOSE(cfg, **kwargs)
